...
/**
 * Service override of \Drupal\samlauth\SamlService.
 *
 * @see \Drupal\samlauth\SamlService
 */
class MultiIdpSamlService extends SamlService {
  ...
  public function login($return_to = NULL, array $parameters = [], $force_auth = FALSE) {
    ...
    $idp_id = $session->get('samlauth_selected_idp', 'default');
    $idp = $this->entityTypeManager->getStorage('samlauth_idp')->load($idp_id);

    $url = $this->getSamlAuth('login', TRUE, $idp)->login($return_to, $parameters, $force_auth, FALSE, TRUE, $config->get('request_set_name_id_policy') ?? TRUE);
  ...
  public function acs() {
    ...
    if (!$idp) {
      throw new \RuntimeException("SAML IdP entity not found for ID '$idp_id'.");
    }
   ...
   */
  protected function processLoginResponse($purpose = 'acs', $initialize = TRUE) {
    $session = $this->requestStack->getCurrentRequest()->getSession();
    $idp_id = $session->get('samlauth_selected_idp', 'default');

    $idp = \Drupal::entityTypeManager()->getStorage('samlauth_idp')->load($idp_id);
    ...
    $config = $this->configFactory->get('samlauth.authentication');
    $auth = $this->getSamlAuth('acs', $initialize = TRUE, $idp);
   ...
   */
  protected function getSamlAuth($purpose = '', $initialize = TRUE, SamlAuthIdp $idp = NULL) {
    if (!$idp) {
      $idp = \Drupal::entityTypeManager()->getStorage('samlauth_idp')->load('default');
    }
    ...
    if ($initialize && !isset($this->samlAuth[$purpose])) {
      ...
      $session = $this->requestStack->getCurrentRequest()->getSession();
      $session->set('samlauth_selected_idp', $idp->id());
      $session->save();
  ...
  protected static function reformatConfig(ImmutableConfig $config, $base_url = '', $purpose = '', KeyRepositoryInterface $key_repository = NULL, SamlauthIdp $idp = NULL) {
    $library_config = [
      ...
      'idp' => [
        'entityId' => $idp->get('idp_entity_id'),
        'singleSignOnService' => [
          'url' => $idp->get('idp_single_sign_on_service'),
        ],
        'singleLogoutService' => [
          'url' => $idp->get('idp_single_log_out_service'),
        ],
      ],
      'security' => [
    ...
    if ($add_idp_encryption_cert) {
      $encryption_cert = $idp->get('idp_cert_encryption');
